import { FaFacebookF, FaGithub, FaLinkedin } from "react-icons/fa";
import { Link } from "react-router";

const Footer = () => {
  return (
    <footer
      className="bg-linear-to-r from-yellow-500 via-orange-400 to-orange-300 
                 bg-size-[200%_200%] animate-gradient-x text-white p-8 "
    >
      <div className="px-4 md:px-14 flex flex-col md:flex-row justify-between items-center">
        <div className="mb-4 md:mb-0">
          {/* logo */}
          <Link to="/" className="flex justify-center items-center gap-4">
            <svg
              width="40"
              height="40"
              viewBox="0 0 40 40"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M20 0C27.4768 0 31.2154 -0.000204921 34 1.60742C35.8242 2.66064 37.3394 4.17577 38.3926 6C40.0002 8.7846 40 12.5232 40 20C40 27.4768 40.0002 31.2154 38.3926 34C37.3394 35.8242 35.8242 37.3394 34 38.3926C31.2154 40.0002 27.4768 40 20 40C12.5232 40 8.7846 40.0002 6 38.3926C4.17577 37.3394 2.66064 35.8242 1.60742 34C-0.000204921 31.2154 0 27.4768 0 20C0 12.5232 -0.000204921 8.7846 1.60742 6C2.66064 4.17577 4.17577 2.66064 6 1.60742C8.7846 -0.000204921 12.5232 0 20 0ZM22 4C13.1634 4 6 11.1634 6 20C6 28.8366 13.1634 36 22 36C30.8366 36 38 28.8366 38 20C38 11.1634 30.8366 4 22 4Z"
                fill="#ffff"
              ></path>
              <path
                d="M36 20C36 25.5228 31.5228 30 26 30C20.4772 30 16 25.5228 16 20C16 14.4772 20.4772 10 26 10C31.5228 10 36 14.4772 36 20Z"
                fill="#ffff"
              ></path>
            </svg>
            <h1 className="font-bold text-2xl ">Plate-Share</h1>
          </Link>
          <p>© 2025 PlateShare. All rights reserved.</p>
        </div>
        <div className="flex gap-4">
          <Link to="https://web.facebook.com/azizulhakim6817/" target="_blank">
            <FaFacebookF size={25} />
          </Link>
          <Link to="https://github.com/azizulhakim6817" target="_blank">
            <FaGithub size={25} />
          </Link>
          <Link
            to="https://www.linkedin.com/in/azizul-hakim-948309237/"
            target="_blank"
          >
            <FaLinkedin size={25} />
          </Link>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
